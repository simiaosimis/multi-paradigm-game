...
function Shot(x_, y_, mx, my, owner){
    this.x = x_;
    this.y = y_;
    this.mx = mx;
    this.my = my;
    ...
    this.owner = owner
    this.path;
    this.iterator = Math.floor(x_/10)*2;
    console.log(this.iterator);
    this.direction = x_ - mx;
...
function updateShot(){
    ...
    for(var i = 0; i<shots.length; i++){
   	 ...
   	 shots[i].x = shots[i].path[shots[i].iterator+1];
   	 shots[i].y = shots[i].path[shots[i].iterator];
   	 if(shots[i].iterator < canvas.width*2 && shots[i].iterator >= 0){
   		 if(shots[i].direction < 0){
   			 shots[i].iterator += 2;
   		 }
   		 else{
   			 shots[i].iterator -= 2;
...
function updateShotCollision(){
    for(var i = 0; i<shots.length; i++){
      for(var j = 0; j<enemy.length; j++){
        ...
        if(Haste.collison(shots[i].x, shots[i].y,x, y, 70 ,70 ) && shots[i].owner == "enemy" && !dead){
...
function spamShot(posX, posY, destX, destY, owner){
    ...
    shots[shots.length-1].path = Haste.line(posX, posY, destX, destY);
}
